...
fn serialize_unit_struct_string() {
    ...
    let expected = Value::BulkString(b"hello".to_vec());
...
fn serialize_unit_struct_u8_redis_int() {
    ...
    let actual = v.serialize(Serializer).unwrap();
    let expected = Value::BulkString(b"5".to_vec());
...
fn serialize_hash_map_strings() {
    ...
    if let Value::Array(values) = actual {
        assert!(values.contains(&Value::BulkString(b"a".to_vec())));
        assert!(values.contains(&Value::BulkString(b"apple".to_vec())));
        assert!(values.contains(&Value::BulkString(b"b".to_vec())));
        assert!(values.contains(&Value::BulkString(b"banana".to_vec())));
    }
}
...
fn serialize_float() {
    ...
    let actual = v.serialize(Serializer).unwrap();
    let expected = Value::BulkString(b"3.14159".to_vec());
...
fn serialize_hash_map_string_u8() {
    ...
    if let Value::Array(values) = actual {
        assert!(values.contains(&Value::BulkString(b"a".to_vec())));
        assert!(values.contains(&Value::BulkString(b"1".to_vec())));
        assert!(values.contains(&Value::BulkString(b"b".to_vec())));
        assert!(values.contains(&Value::BulkString(b"2".to_vec())));
    }
}
...
fn serialize_enum() {
    ...
    let actual = v.serialize(Serializer).unwrap();
    let expected = Value::Array(vec![
        Value::BulkString(b"Orange".to_vec()),
...
fn serialize_byte_buf() {
    ...
    let expected = Value::Array(vec![
        Value::BulkString(b"48".to_vec()),
        Value::BulkString(b"49".to_vec()),
        Value::BulkString(b"50".to_vec()),
        Value::BulkString(b"51".to_vec()),
    ]);
    assert_eq!(expected, actual);
}
